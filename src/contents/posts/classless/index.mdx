...
プロトタイピングやデモサイトなど、簡単なサイトを素早く構築することができます。
...
ライト/ダークモードのどちらか、ユーザの環境にあわせて自動でライト/ダークを切替る、3つのテーマから選べます。

Next.jsで使うのは簡単で、適用したいページに`Head`を`import`して、使いたいテーマのCSSのURLを下記のように追加するだけです。
...
Wataer.cssと同様に、`Head`に以下を追加すれば使えます。
...
`@stackhackerio/classless`はTailwindcssのプラグインなので、Tailwindcssのユーティリティでカスタマイズすることができます。
...
`@stackhacker/classless`をすぐに使えるようにスターターをご用意したのでご活用ください。
